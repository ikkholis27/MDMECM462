...
uint32_t fota_write_start_addr=(uint32_t)0x08010000U, fota_write_end_addr=(uint32_t)0x0801F000U;
/* calculate the number of page to be programmed/erased */
uint32_t FotaPageNum;
/* calculate the number of page to be programmed/erased */
//uint32_t FotaWordNum;

static char MCU_SOFT_VERSION[]="MDA_V1.2E38A1A";
uint32_t *ptrd;
...
uint8_t modecfg, cnttimer_modecfg;
...
static char cmp_msg8[]="AT+MFW1";
static char cmp_msg9[]="AT+REOPS";
...
static char uart1_tx[2200] = {0};
// Uart1 Vars
int uart1_index = 0, socket_id=0, cmpnum=0;
...
float tempfloat, tempfloat1, tempfloat2;
double tempdbl, tempdbl1, tempdbl2;
...
/*			case 6: cmpnum = strcmp(smsmsg, cmp_msg6);
			...
			case 8: cmpnum = strcmp(smsmsg, cmp_msg8);
							...
							break;
			case 9: cmpnum = strcmp(smsmsg, cmp_msg9);
							if(cmpnum==0) {
								val=26;
						 };
							break;
...
//		printf("AT+CMGD=1,4\r\nsmsnum1 %s\r\n", smsnum1);
		...
		delay(2);
		 process_sms=0;
		 if(numinwhitelist!=0 && fmc_buf[76]==1){
			process_sms=1;			 
		 }
		 else if(fmc_buf[76]==0 || fmc_buf[76]==255){
			process_sms=1;			 			 
		 }
//		 if(numinwhitelist!=0){
		 if(process_sms!=0){
...
//											printf("\r\ncase %d OK\r\n", cmp_smsmsg); // DEBUG
					 ...
					 case 18 : 	memset(smsmsg,0,sizeof(smsmsg));
											sprintf(smsmsg,"AT+MGNET=%d",network);
//											send_sms();
											send_sms_withsmsnum();
											delay(30);
											sim_flag=sim_flag_old;
...
//											printf("\r\ncase %d OK\r\n", cmp_smsmsg); // DEBUG
					 ...
					 case 25 :	memset(smsmsg,0,sizeof(smsmsg));
											sprintf(smsmsg,"AT+MFW1 OK\r\nREBOOTING NOW\r\nPlease Wait in 5 Minutes");
//											send_sms();
											send_sms_withsmsnum();
											delay(50);
											// Jump to user application 
...
//											printf("\r\ncase %d OK\r\n", cmp_smsmsg); // DEBUG
					 ...
					 case 26 	: memcpy(smsmsg1, smsmsg, sizeof(smsmsg));
											memset(smsmsg,0,sizeof(smsmsg));
											sprintf(smsmsg,"%s. \r\nWORKING. Reopening Socket. Please Wait in 3 mins!",smsmsg1);
											if(numinwhitelist!=0)send_sms();
											else send_sms_withsmsnum();
											delay(30);
											uart1_index=0;
...
void uart0_handle(void)
{
    ...
    if(RESET != usart_interrupt_flag_get(USART0, USART_INT_FLAG_RBNE)){
			...
			rx_cnt++;
			if(rx_cnt>1 && (sim_flag==27 || sim_flag!=4 || sim_flag!=14)){
				if(ch=='D' && uart0_rxbuf1[rx_cnt-2]=='E'){
					modecfg=1;
					sim_flag=28;
				}
				else {
...
//				}
			...
			else if(rx_cnt>1 && (sim_flag==27 || sim_flag==4 || sim_flag==14)){
				if(ch=='L' && uart0_rxbuf1[rx_cnt-2]=='D'){
					modecfg=1;
					sim_flag=28;
				}				
			}
...
//			if(TESTMODE)   usart_data_transmit(USART0, ch);
...
//				sim_flag=14;
			...
			if(sim_flag==13 && uart1_rxbuf[uart1_index-1]==',' && uart1_rxbuf[uart1_index-4]==':'){
...
//						if(TESTMODE)   usart_data_transmit(USART0, callernum[j]);
	...
	delay(20);
		...
		numinwhitelist=1;
		 ...
		 process_sms=0;
		 if(cmpnum==0 && fmc_buf[76]==1){
			process_sms=1;			 
		 }
		 if(fmc_buf[76]==0 || fmc_buf[76]==255){
			process_sms=1;			 			 
		 }
		memset(smsnum1,0,sizeof(smsnum1));
		memcpy(smsnum1, callernum, sizeof(smsnum1));
//		printf("extractnum Callernum %s %d\r\n",callernum, cmpnum);
		 callingcnt++;
		if(callingcnt>3 && cmpnum==0){
			reboot_by_calling();
		}
}
...
// ------------------------ FOTA FMC funcs ----------------------------------------
/*void fota_fmc_backup(void)
{
    uint32_t i;
		char c;

    ptrd = (uint32_t *)ApplicationAddress;
		FotaWordNum = ((0x8011800U - ApplicationAddress) >> 2);
    // check flash whether has been programmed
    for(i = 0; i < FotaWordNum; i++){
			data1=*ptrd;
			printf("%u\r\n",data1);
			delay(5);
            ptrd++;
...
/*
void fota_to_memory(int addr_src, int addr_dest) {
    uint32_t val = 0;
			// get current character then increment
			uint8_t byte = temp_global[addr_src+6];
			// transform hex character to the 4bit equivalent number, using the ascii table indexes
			if (byte >= '0' && byte <= '9') byte = byte - '0';
			else if (byte >= 'a' && byte <='f') byte = byte - 'a' + 10;
			else if (byte >= 'A' && byte <='F') byte = byte - 'A' + 10;    
			val = (val << 4) | (byte & 0xF);
			byte = temp_global[addr_src+7];
			// transform hex character to the 4bit equivalent number, using the ascii table indexes
			if (byte >= '0' && byte <= '9') byte = byte - '0';
			else if (byte >= 'a' && byte <='f') byte = byte - 'a' + 10;
			else if (byte >= 'A' && byte <='F') byte = byte - 'A' + 10;    
			val = (val << 4) | (byte & 0xF);
			byte = temp_global[addr_src+4];
			// transform hex character to the 4bit equivalent number, using the ascii table indexes
			if (byte >= '0' && byte <= '9') byte = byte - '0';
			else if (byte >= 'a' && byte <='f') byte = byte - 'a' + 10;
			else if (byte >= 'A' && byte <='F') byte = byte - 'A' + 10;    
			val = (val << 4) | (byte & 0xF);
			byte = temp_global[addr_src+5];
			// transform hex character to the 4bit equivalent number, using the ascii table indexes
			if (byte >= '0' && byte <= '9') byte = byte - '0';
			else if (byte >= 'a' && byte <='f') byte = byte - 'a' + 10;
			else if (byte >= 'A' && byte <='F') byte = byte - 'A' + 10;    
...
//		check_set_fmc();
	...
	else if(smsmsg[6]=='1' && smsmsg[5]=='W' && smsmsg[4]=='F' && smsmsg[3]=='M'){
		printf("AT+MFW1 OK\r\n");
		delay(20);
		fmc_buf[255]=1;
		fmc_buf[254]=1;
...
//		sim_flag=98;
	...
	// MFW2
		...
		printf("AT+MFW2 OK. Current FW: %s. Rebooting Now\r\n", MCU_SOFT_VERSION);
		delay(20);
		fmc_buf[255]=1;
		fmc_buf[254]=1;
		fmc_data_backup[255]=1;
...
//		jump_to_address_fw2();
		...
		printf("AT+MFW1 OK. Current FW: %s. Rebooting Now\r\n", MCU_SOFT_VERSION);
		delay(20);
		fmc_buf[255]=1;
		fmc_buf[254]=1;
		fmc_data_backup[255]=1;
...
void check_shrst_fmc(){
	...
	if(hour_srst==0 && min_srst==0) hour_srst=24;
...
//				printf("fmc_buff[65] %d %d %d %d %d %d %s\r\n", fmc_buf[65], mintotal_srst, min_srst, hour_srst, tempint, end, smsmsg);
			...
			// HRST
				...
				if(tempint>=10080 || tempint<=0) {
					...
					tempfloat = (float)min_hrst/60;
					tempfloat1 = ceil(tempfloat);
					tempfloat2 = tempfloat1 - 1;
					tempint = min_hrst;
					min_hrst = tempint%60;
					fmc_data_backup[103]=tempfloat2;
					hour_hrst = fmc_data_backup[103];
				}
...
//						led_off(LEDB);
	...
	for(int p=3;p<sizeof(uart1_rxbuf);p++){
		if(uart1_rxbuf[p-3]=='C' && uart1_rxbuf[p-2]=='C' && uart1_rxbuf[p-1]=='L' && uart1_rxbuf[p]=='K') cntcclkerr=0;
		else cntcclkerr++;
	}
	if(sim_flag_old<3) {
		while(1){
...
//		nvic_vector_table_set(NVIC_VECTTAB_FLASH,0x10000);
    ...
    rcu_periph_clock_enable(RCU_GPIOB);
		...
		if(fmc_buf[240]>=100) jump_to_address_fw2();
		else {
			check_apn_fmc();
			check_port_fmc();
			check_wl1_fmc();
